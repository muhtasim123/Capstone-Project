...
function checkAlbumSelected($currprof, $curralbm, &$redirurl, &$redirtime) {
   ...
   if(!isset($_REQUEST['selectedAlbum'])) {
       ...
       echo "<a href='albumgallery.php?profileid={$currprof}&albumname={$curralbm}'>Click here if the browser is not automatically redirecting in {$redirtime} seconds.</a>";
...
if(!isset($_REQUEST['gallery'])) {
   ...
   case 'Copy':
       ...
       checkAlbumSelected($currprof, $curralbm, $redirurl, $redirtime);


       copyToAlbum($mysqli, $currprof ,transformGalleryURL($_REQUEST['gallery']), $_REQUEST['selectedAlbum'] );
   ...
   case 'Play From First Selected':
       ...
       $redirurl= "profiles.php?profileid={$currprof}&albumname={$curralbm}&startpicture={$firstSelect}";
...
function copyToAlbum($connect, $currprof, $tableURLs, $album) {
   ...
   $stmt->bind_param('iss', $currprof, $newurl, $album);
...
<meta http-equiv="refresh" content="<?php echo $redirtime; ?>; URL='<?php echo $redirurl; ?>'"/>
